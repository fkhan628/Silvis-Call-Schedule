...
PRECONDITION (review F3; met by Prompt 20 R2): a follower changes or stops his own mail with his own switches - Settings >
Notification settings gives him the three switches and the reminder hour, saved to his `notification_preferences` row by `profile_id` (`POST
?on_conflict=profile_id`, through the `prefs_own` profile_id path revision o adds), and the follower e-mails' footers say
"... use Notification settings under Settings in the app" (F3's said "ask the scheduler"). The switches work only once
revision o is applied (before it the card says "Follower settings are available after the next update" and writes
nothing) - so the order stays: the client build, revision o, then these two. If the switches are not on the live build,
do not deploy.
...
self-update pin); a role change away from viewer / coordinator keeps clearing `follows`. The PRECONDITION above is met by the
follower's own prefs editor (the three switches and the reminder hour, saved by `profile_id` - Prompt 20 R2), built on this branch for the same push. These two deploys
