   ...
   the importer's denylist gate (Prompt 12 item F) here. */
{
  ...
  check("index-source.html: Accept & Publish stores lastGenerateFromDiagnostics(pv.diagnostics, ...) only after the CAS write succeeded (r.ok); the autosave watches lastGenerate; the board reads lastGenerate.weekendKinds", () => {
    ...
    assert.ok(/\}, \[loaded, surgeons, surgeonRules, groupRules, holidays, settings, lastPublished, lastGenerate, saveTick\]\);/.test(appSrc), "the autosave's blob leg lists lastGenerate in its dependencies (saveTick is Prompt 16 A3's re-fire counter; A4 keyed the blob leg on the setup state alone)");
