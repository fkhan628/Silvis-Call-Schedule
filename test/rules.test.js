...
eq(sched["2026-10-15"].backupLocked, true, "BK 9/23: 10/15 backup is Burchett, locked (the primary stays open and unlocked)");
eq(sched["2026-10-24"].backupLocked, false, "null backup slot is never locked (10/24: both roles open in the seed)");
...
eq(ctx.activeIds.filter(id => R.eligibility(ctx, OCT15, B, id, { ignoreLocks: true }).ok), [KHAN, BURCHETT, ACTON, FIERCE], "10/15 backup: open to everyone the day rules used to block (9/22; the seed's BK 9/23 lock on Burchett is ignored here)");
eq(ctx.activeIds.filter(id => R.eligibility(ctx, OCT15, B, id).ok), [BURCHETT], "BK 9/23: on the seed's rows 10/15 backup is Burchett's locked manual edit - everyone else reads slot-locked");
ok(R.eligibility(ctx, OCT15, B, BURCHETT).lockHolder === true, "BK 9/23: Burchett is the 10/15 backup lock holder on the seed");
...
eq(tOct.backup, 2, "Acton October backups: 6, 8 (10/20 went to Burchett - BK 9/23, a locked manual edit mirrored in the seed)");
eq(tOct.total, 12);
eq(R.talliesFor(ctx, BURCHETT, "2026-10").backup, 5, "BK 9/23: Burchett October backups on the seed: 10/12 (his 9/17 plan) + 10/9, 10/15, 10/20, 10/22 (his 9/23 email)");
