...
// Prompt 13 / P13R (9/23): the SAME two slots held through a surgeon acting for himself - Khan 10/21 backup with row
// source "claim" (claim_open_slot), Acton 10/24 primary with row source "trade" (apply_trade); neither SQL path sets
// a lock flag (test/fixtures/claim-fixed-2026-10.json). GEN_PERSON_FIXED_SOURCES = ["claim", "trade"] makes every
// held role of such a day fixed in BOTH modes exactly like a lock: byte-identical on the output, lock flags untouched,
// counted in fixedSlots, never a hard violation (Acton's off-list 10/24 lands in fixedViolations), the open slots
// around them filled or listed. The block above pins the contrast: source "manual" IS regenerated by a plain generate.
{
  const FIXC = JSON.parse(fs.readFileSync(path.join(__dirname, "fixtures", "claim-fixed-2026-10.json"), "utf8"));
  eq(GEN.GEN_PERSON_FIXED_SOURCES, ["claim", "trade"], "GEN_PERSON_FIXED_SOURCES is claim + trade (manual is NOT fixed - the day editor has a lock toggle)");
  const inputC = SA.seedToContextInput(seed, { eastDerived: DERIVED, eastBusyDays: { [KHAN]: KHAN_BUSY }, eastFeedCoverage: EAST_COVER, eastForecast: { [KHAN]: FORECAST } });
  Object.keys(FIXC.schedule).forEach((d) => { inputC.schedule[d] = JSON.parse(JSON.stringify(FIXC.schedule[d])); });
  const ctxC = R.buildContext(inputC);
  if (ctxC.warnings.length) fail("buildContext warnings (claim-fixed fixture): " + JSON.stringify(ctxC.warnings));
  CUR.range = "claim-fixed fixture " + BF.start + ".." + BF.end; CUR.seed = 1; CUR.day = "2026-10-24";
  eq([FIXC.schedule["2026-10-24"].primary, FIXC.schedule["2026-10-24"].primaryLocked, FIXC.schedule["2026-10-24"].source, FIXC.schedule["2026-10-21"].backup, FIXC.schedule["2026-10-21"].backupLocked, FIXC.schedule["2026-10-21"].source], [ACTON, false, "trade", KHAN, false, "claim"], "fixture: Acton 10/24 primary (trade) and Khan 10/21 backup (claim), neither locked");
  [["generate", {}], ["fill-open-only", { fillOpenOnly: true }]].forEach(([modeName, extra]) => {
    const outC = GEN.generate(ctxC, BF.start, BF.end, Object.assign({ seed: 1, bestOf: 2 }, extra));
    const DC = outC.diagnostics, c24 = outC.schedule["2026-10-24"], c21 = outC.schedule["2026-10-21"];
    CUR.range = "claim-fixed fixture " + modeName; CUR.day = "2026-10-24";
    eq(DC.mode, modeName, "fixture: mode");
    eq(DC.fixedSlots, bfFixed + 2, "fixture (" + modeName + "): the claimed backup and the traded primary count as fixed");
    eq([c24.primary, c24.primaryLocked, c24.source, c24.note], [ACTON, false, "trade", FIXC.schedule["2026-10-24"].note], "fixture (" + modeName + "): 10/24 primary - the traded, unlocked Acton stays as written");
    eq([c21.backup, c21.backupLocked, c21.primary, c21.primaryLocked, c21.source], [KHAN, false, ACTON, true, "claim"], "fixture (" + modeName + "): 10/21 - the claimed, unlocked Khan backup stays, the locked Acton primary too");
    ok(c24.backup && c24.backup !== ACTON, "fixture (" + modeName + "): 10/24 backup is filled by someone else (got " + c24.backup + ")");
    eq(DC.hardViolations, [], "fixture (" + modeName + "): fixed slots are facts, never hard violations");
    ok(Array.isArray(DC.fixedViolations) && DC.fixedViolations.some((v) => v.day === "2026-10-24" && v.role === P && v.id === ACTON && v.reasons.some((r) => r.indexOf("whitelist-month") === 0)), "fixture (" + modeName + "): Acton's off-list traded 10/24 is a fixedViolation (whitelist-month), got " + JSON.stringify(DC.fixedViolations));
    eq(DC.lockViolations.map((l) => l.day + " " + l.role + " " + l.id), expectedLockViolations(BF).map((l) => l.day + " " + l.role + " " + l.id), "fixture (" + modeName + "): lockViolations unaffected (locked slots only)");
    ok(!DC.warnings.some((w) => /generator bug/.test(w)), "fixture (" + modeName + "): generator reports its own bug");
    const openC = []; bfDays.forEach((d) => ROLES.forEach((role) => { const e = inputC.schedule[d]; if (!(e && (e[role] || (role === P && e.externalCover)))) openC.push(d + " " + role); }));
