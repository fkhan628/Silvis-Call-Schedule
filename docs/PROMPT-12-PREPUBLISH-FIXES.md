...
body is quoted `$pub$ … $pub# Prompt 12 — Pre-publish fixes + the 9/22 rule amendments (v4)
...
Pages with the next push of `main`).
 ...
 (a `$` in a note cannot end it); the final `SELECT` reports `stamped_at` and counts the
