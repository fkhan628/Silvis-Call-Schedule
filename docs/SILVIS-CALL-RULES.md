...
| Period | **This round: generate through the end of 2026** (range 2026-11-02 → 2027-01-03 so the New Year's weekend is covered). **After that, Generate offers 3-, 6-, 9- and 12-month presets** from the last published day. **Sep 14 – Nov 1 stays exactly as the emails describe it** (locked import, §7); the rules below generate from November onward. ⟶ **9/22 late (Prompt 12 AB): the in-app default range now starts at the first open slot on or after today, not after the last published day** — locks are never touched, so one run from there fills the open October backups (the first is 10/7), the 10/15 and 10/24 primaries, every 11/5-type hole and the milestone range; the preset ends are unchanged. | Khan 9/21, **9/22** |
...
- ⟶ **9/22 (Prompt 12 T; Faraz's final word ~15:25): "The original Fierce plan should stand — that rule of his will follow
  ...
  **Mon 11/16 is Faraz's explicit decision** (~15:35: "11/16 stays Fierce — that was my decision, not an inference"; an ordinary
  ...
  (`pendingDeltas`). **Import side:** a the ER-panel author/email entry that collides with one of his derived weeks is dropped with a
...
11. ~~**October's open backups + 10/24 primary** — fill by generator (fill-open-only over 10/15 – 11/1: `scripts/preview-generate.js --backfill`, Prompt 12 T) or by hand before publishing? The backfill section of the preview lists a candidate and the alternatives per open slot; Faraz decides. (The import has eleven open backups in that range, not eight — item 14.)~~ ⟶ answered 9/23: the fill-open-only backfill ran and was published (`docs/PUBLISH-2026-09-23.md`); four October backups were then re-locked to Burchett by hand 9/23 08:38 (10/9, 10/15, 10/20, 10/22 — `docs/REPORT-BURCHETT-OCTOBER-2026-09-23.md`).
...
20. **Offer modes per surgeon for the first period (Nov 2026 – Jan 2027) — defaults set 9/23 (Prompt 14 P5/P6),
    ...
    over Nov–Jan would still read the 79 offers, status does not gate the engine). The next answer window is the second
    period, Feb 2027 – Apr 2027 (freeze 12/21, publish by 1/4): its row carries no rules-only list and no modes (the
    preset's shape) — each surgeon chooses in the painter, or Faraz lists them in `offerPeriods[1]` before an apply.**
