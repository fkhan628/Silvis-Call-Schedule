...
- ⟶ **9/22 (Prompt 12 C) — conflict report:** when a refresh adds or moves a derived week, the East feed card ("Conflicts with the
  ...
  raises nothing (backup is open to him every day); the day editor's "East week -> Silvis <role> (derived)" line, Setup > East feed's derived-weeks list (the month grid's E badge came off on 9/25, item E2) and `diagnostics.derivedYields` cover it. Locked rows
