...
open primaries 10/15 and 10/24. The fill-open-only pass is `generator.generate(ctx, start, end, { fillOpenOnly: true })`: every
...
17. **East vacations — how far out should an unreviewed range block him? (Prompt 15, 9/23).** An unreviewed Davenport
    ...
    penalty beyond that and the nag as today? If yes it is one data key in `groupRules.eastFeed` and a
    small rules change, not a code branch per surgeon. Also still open from the same prompt: whether `claim_open_slot`
    should read the feed + the reviews (today the derived vacation is enforced client-side only), and that a
