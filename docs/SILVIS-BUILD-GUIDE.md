...
| `trade_insert_guard` | a member's one-way trade is refused by the client only | a `'give'` with a return leg is refused for every caller (`TRADE_INELIGIBLE: a give is one-way ...`); a member's `'give'` with no return leg is accepted; the scheduler may still record a one-way `'trade'` and may insert a `'give'`; from := me / same-surgeon / roster names unchanged (a member's give of a day he does not hold lands from him and `apply_trade` refuses it, TRADE_STALE). The member return-leg refusal (`TRADE_INELIGIBLE: a trade needs a return shift ...`) is **deferred** to the prepared follow-up `sql/migrations/2026-09-25-member-trade-return-leg.sql` (report-first, NOT applied; apply only after the `client_versions` min_version bump to the Prompt 19 build and a day for old builds to drain) | `submitTradeRequest` — the live build's one-way UNIT TAIL rows (rows 2..n of a whole-unit trade with one return day) keep landing after this file; the follow-up would refuse them until EVERY installed app runs the client step, which sends them as `'give'` (a stale app's announced unit proposal could then be applied as a unit split - hence the split and the follow-up's gate; SCHEMA-REVIEW has both sections) |
...
| `notification_preferences` key | `person_id` primary key | `id` primary key; `person_id` UNIQUE + nullable; `profile_id` UNIQUE → `user_profiles` on delete cascade; exactly one of the two (`notification_preferences_one_owner`) | `saveNotifPref` upserts with `?on_conflict=person_id` (through `notifPrefsDb.save({ personId })`, P20 R2); `send-notification` / `daily-reminder` read the surgeons' rows by `person_id` with the service role (unchanged) and a follower's row by `profile_id` through the `@followers` block (v8 / v6) — the row his R2 switches write |
| `prefs_own` | own `person_id` or scheduler | also `profile_id = auth.uid()` | a follower's own prefs row (Settings > Notification settings, Prompt 20 R2: read by `profile_id=eq.<id>`, saved with `?on_conflict=profile_id`) |
...
Decisions (Faraz 9/25, recorded in the SCHEMA-REVIEW section): followers **do** get the publish e-mail (`schedule_published`); the `user_profiles_self_insert` pin (`follows = '[]'`) and the self-update pin **stay** - a profile never chooses whom it follows; a role change away from viewer / coordinator **keeps clearing** `follows`. The follower's own prefs editor (three switches and the reminder hour, by `profile_id`) is built (Prompt 20 R2, the "A follower's notification settings" bullet in the UI section) and ships in the same push, and revision o ships in ONE rollout with the member return-leg follow-up: push -> min_version bump -> 24 h with every heartbeat on that build -> both migrations (probes + verify-rls) -> `send-notification` v8 + `daily-reminder` v6.
...
- **Month grid cell:** two lines — `P Burchett` / `B Acton` — colored per surgeon; open slot = red "OPEN" (the ER-panel author's convention); weekend units get a subtle bracket; locked slots show a padlock. **No East-derived / forecast marking on the month grid, for anyone (item E2, Faraz 9/25: "remove the east specific markings on the calendar because it makes it busy ... just want to improve readability"):** the E (East-derived week) and F / f (East forecast at / above threshold, 20% or more) cell badges, their two legend lines and the "East-derived: ..." hover-title bit are gone from the grid - the scheduler included (item E, 9/24, had kept them for the scheduler behind `eastBadgesVisible = isScheduler && !isPublicMode`; the flag and the grid-only `forecastByDay` map went with them). `nBadges` now counts the confirm "?" badge alone (the holiday-label gutter stays at its 30px floor). The scheduler reads the East status where it always was: the day editor's East lines (`eastStatusLines` - "East week -> Silvis <role> (derived)", "East forecast NN% (treated as busy / below the threshold)") and Setup > East feed (its derived-weeks list `east-derived` and forecast-days line `east-forecast-days` are the at-a-glance view), both unchanged. E2 left the editor's East lines visible to every role (`?public=1` included) and the East-vacation diamonds, their legend line and "East vacation:" hover (Prompt 15), My schedule / Following's E / O / F / EV badges and the coverage strip's forecast / feed items as they were. **Item E3 (Faraz 9/25: "East and override details are the scheduler's business"):** ONE flag, `eastDetailsVisible = isScheduler && !isPublicMode` (the pattern item E used), makes these the scheduler's only - surgeons, coordinators, viewers, followers and `?public=1` get none of them, with no empty heading or East word left in a title: the day editor's East lines (`eastStatusLines` is not even built; the East-vacation review state and its diamond included - which also ends the public page's "unreviewed" Davenport-vacation line, since public mode never loads the reviews) and its "locked holder breaks: ..." line (override detail); the grid's East-vacation diamonds with their "East vacation:" cell / dots hover bits (`eastVacByDay` is empty) and the legend line; the coverage strip's "forecast-busy as primary", "East feed through" and "unreviewed East vacations" items (the last was the scheduler's and Khan's - Faraz is both, so nothing is lost); Totals' East days column (header tooltip, cells, the fairness row's " + N East P-week", the footnote; the cap tooltips word a counted East day neutrally as "the other days this cap counts" - the numbers and the over-cap flag are unchanged for everyone) and the Totals CSV, which follows the screen: without the flag the "East days" column is left out (21 columns) and the cap-count header reads "Counted vs cap"; the scheduler's file is unchanged, column for column. A derived-week row's `source` "east-derived" reads "generated" in the editor for non-schedulers. Unchanged on purpose (decided 9/24): My schedule / Following's badges, Khan's `?east=1` calendar feed and the office digest's Davenport section. Overrides no longer write reason codes into `schedule_days.note` (anon-readable; CLAUDE.md: notes there carry no reasons) - see the day editor bullet below. Still showing East wording to non-schedulers, not in E3's list: the Open shifts board's eligible-chip hovers, the Take button's title, the claim sheet's "Allowed, shown so you know" list and the claim-gate toast; the trade card's reasons; the offer painter; the East-feed-cache toast and the "Couldn't load the East vacation reviews" toast (a real read failure, every signed-in role); the importer's 11/9-11/16 seed notes ("East primary week ...") in the grid hover, the NOTE marker, the editor's Note line and the share page - data, a pending decision for Faraz (removing them needs the seed AND the live rows, or the next import brings them back). Display only - rules (`ctx.derivedByDay`), the feed, the forecast and the generator are untouched (pins: `test/data-layer.test.js` "Item E2 (9/25)" and "Item E3 (9/25)"; smoke "Item E3 (surgeon | Khan as surgeon | coordinator | viewer | ?public=1)" - each page's role read from the page first (its Time off tab, the office's offers card); no East text (case-insensitive), title or `[data-eastvac]` in the day editor (a derived day, an unreviewed East-vacation day, 10/15's locked holder, the first served `east-derived` row - "Source: generated"), the grid + legend, the coverage strip or Totals (month, fairness, every CSV line at 21 fields), non-vacuous against that page's own rules context - except the public grid, which never drew an East-vacation marker (public mode, 9/23), and the strip's East-vacation item, which only the scheduler and the East-code person ever saw: "Khan as surgeon" is the page that proves its gate; a day's note is left out and reported by day as the pending decision above - and "Item E3 (scheduler)" - every one of them still shown (22-field CSV lines, the footnote, Fierce's " + N East P-week"); "Item E3 (override save)" - the block-member step's override writes no tag or reason into the note while `schedule.override` / `schedule.day_edit` carry the reasons; smoke "Item E2 (scheduler, 1180)" / "Item E2 (scheduler, day editor)" - the editor's derived-week and forecast lines checked, day by day, against the rules context on the first month from November 2026 with a derived week and the first with a forecast day of 20% or more (exact match both ways; nothing East in 12 months fails) - / "Item E2 (scheduler, 390 dark)" / "Item E (surgeon, 390 dark|light)" / "Item E (?public=1)").
...
- **Day editor** (click a cell): set primary/backup from a dropdown that shows eligibility — eligible names first, ineligible greyed with the reason; lock toggle; note. Backup lists everyone (9/22: backup is open unless `backupOptOut`). Outside surgeons (M) sit under their own "Outside surgeons" heading for both roles; picking one locks the role and saves `source: "manual-external"`. A Fri/Sat/Sun candidate who already holds the other two block days is judged as a block member (small items 9/22 — Fierce can complete a Fri–Sun block by hand); the same holds for a block-style receiver of a whole Fri–Sun block in the trade path. The editor **fails closed**: a thrown eligibility check makes the option ineligible with the error as its reason and disables Save until the rules evaluate again. **Overrides (Item E3, Faraz 9/25):** an override assigns anyway and its reasons go to the audit log only - `schedule.day_edit` (`detail.overrides`) and `schedule.override` ("Override on M/D: <Name> <role> despite <reasons>"); `saveDayEdit` no longer puts an `[override: ...]` tag in front of the day's note (`schedule_days` is anon-readable). The audit rows being the only copy of the reasons now, `logAudit` answers true / false (every other caller still ignores it) and when BOTH rows of an override are refused / fail the scheduler gets an error toast with the reasons - also while the session has expired, since audit rows are never re-sent after the sign-in (one written row still keeps them; a console line names it); as before E3, the audit rows go out before the autosave's CAS sync, so a sync conflict leaves them standing for an override that did not land (accepted). An old tag is still stripped on the day's next save (and `awaitingConfirmation` still skips one); the one live tag (10/15, written by the app 9/25 15:24 CDT) waits for Faraz's go: a snapshot-first compare-and-swap that removes only the tag.
...
- **Setup > Users: Follows (Prompt 20 F2, 9/24):** the admin chooses whom a viewer or coordinator account follows in a "Follows" column between Roster link and Since: one chip per *active* roster surgeon in roster order (code on the chip, name in the title, `aria-pressed`, `data-testid="user-follow-<uuid>-<id>"`), plus any id the row follows that the active roster no longer lists, placed after them so it can still be cleared. Surgeon, scheduler and admin rows show a muted "-" (`user-nofollow-<uuid>`) and no control. A click saves through the existing path, `onSave` -> `saveUserProfile` (admin only; `PATCH user_profiles?id=eq.<uuid>` with `return=representation`), one save per row at a time, and its `users.link` audit summary names the ids: "follows s2, s5" or "follows none". Before the PATCH, `helpers.followsPatch` checks every save. It refuses `follows` unless the role after the patch is viewer or coordinator, and it refuses anything but distinct roster ids (or ids the row already follows). A role change away from viewer or coordinator on a row that follows someone adds `follows: []` to the same PATCH, because a list left on a row with no control would be invisible; the audit reads e.g. "role -> surgeon, follows none". The Setup > Users read stays `user_profiles?select=*`, since naming the column would answer 400 before revision o is applied. `helpers.followsColumnState(rows)` then says "present" (any row carries the key; the column is NOT NULL with a default), "absent" (rows without it) or "unknown" (no rows). While it is absent the card shows `users-follows-pending`, the chips are disabled and `toggleFollow`'s own guard refuses the save, so the page loads normally and writes nothing. Proof: `test/data-layer.test.js` [P20 F2] (the helpers' behaviour, the `saveUserProfile` gate and summary, the source pins) and the smoke's Setup > Users pass at 390 px (`setup-users-follows-390.png`).
- **What a follower gets (Prompt 20 F3, 9/24):** read-only, driven by `user_profiles.follows` and the follower's OWN prefs row (keyed by `profile_id`; no row = every flag on, the default hour). In the app, `helpers.followedIdsOf(profile, rosterIds)` gives `myFollows` (a viewer / coordinator with no roster link only; never after a failed profile read): `notifVisibleTo(..., { follows })` shows him exactly what each followed surgeon reads; the Mine tab reads "Following" and renders one `following-card` per followed surgeon through the same `daysBlock(who, own)` as Mine (hero + 90-day list; `own` false = no trade button, no offer tag, no painter, no vacation entry); Settings > Live calendar sync lists each followed surgeon's `calendar-sync?surgeon=<CODE>` with Copy (`follow-sync`) above the full feed; Settings > Notification settings gives him his own switches (Prompt 20 R2 - the next bullet; F3's `notif-follower-note`, which sent him to the scheduler, is gone). `profilePollMerge` also compares `follows` (order-aware), so an added or removed follow reaches an open app on the next 60-second poll. His one client write path is his own prefs row (R2). In the functions (repo only until deployed after revision o - `edge-functions/README.md` section 3): the `@followers` plain-JS block (identical in both) - `daily-reminder` reminder mode mails "Reminder: Dr. <Name> is on primary call at Silvis tomorrow (Fri 10/9), backup <Name>" once per followed surgeon on call tomorrow at the follower's own hour and `shift_reminders_email`, and its dryRun answers `followers`; `send-notification` adds, after every gate and on the follower's own flag, the followers of `followerUniverse` - the notice's own parties (the trade row's two, the claimer; never a scheduler-linked copy), a targeted list, or for a broadcast every follower - for `trade_*` (a give included), `shift_claimed`, `open_shifts` and `schedule_published` (the publish broadcast reaches linked persons only, so a follower was never in it), and answers the `followers_*` counts (`followers_added` to an admin / scheduler caller only). The follower frames' footers point him at his own switches ("... use Notification settings under Settings in the app" - Prompt 20 R2; F3 said "ask the scheduler"), which is what the section 3 deploy PRECONDITION asks for. Both read the accounts with `select=*` (no follower before revision o, never a 400). Proof: `test/data-layer.test.js` [P20 F3], `test/edge-functions.test.js` P20 F3, the smoke's follower session at 390 px.
- **A follower's notification settings (Prompt 20 R2, Faraz 9/25):** Settings > Notification settings gives a follower (a viewer / coordinator with follows and no roster link) the same three switches a surgeon has - schedule e-mails (publish, open shifts, shifts taken), trade e-mails, the day-before reminder - and the reminder hour (Central; default 5:00 PM), under a line naming whom he follows (`notif-follower-prefs`, its `data-state` the read state). His row is keyed by `profile_id` = his own `user_profiles.id` (= `auth.uid()`, what `prefs_own` checks): `config.js` `notifPrefsDb.loadFollower(id)` reads `notification_preferences?select=*&profile_id=eq.<id>` with his JWT and `saveFollowerPref` (the same 500 ms debounce as a surgeon's) sends `notifPrefsDb.save({ profileId })` = ONE `POST ...?on_conflict=profile_id` whose body names `profile_id` and carries no `person_id` key (the row's `person_id` stays null - `notification_preferences_one_owner`); a surgeon's save is `notifPrefsDb.save({ personId })` = `?on_conflict=person_id`, never naming `profile_id` (the row is `helpers.notifPrefSaveRequest`'s for both). The switches are enabled - and a save can be sent - only after his row was read (`ok`; no row = every switch on). Before revision o the column is missing and the read answers PostgREST's HTTP 400 `42703` ("column notification_preferences.profile_id does not exist", observed live 9/25): the card says "Follower settings are available after the next update" (`notif-follower-unavailable`) with every control disabled and writes nothing; any other failed read says "Couldn't load your saved notification settings" (`notif-follower-load-failed`), also disabled - never a silent empty state. No audit entry for either follower role (`audit_insert` refuses a viewer; a coordinator, whom it would accept, is not audited either - one follower path, a named deviation from the surgeons' `prefs.save` entry). The roster prefs load skips a row without `person_id` (his own row, which `prefs_own` also returns). The e-mails' follower footers now say to use Notification settings under Settings in the app. Proof: `test/data-layer.test.js` "P20 R2" (the save / read behaviour through `notifPrefsDb`, the surgeon pin, the card pins), `test/edge-functions.test.js` (the two footers), the smoke's follower session at 390 px in both themes (the three switches read from his served row, one toggle = one POST `?on_conflict=profile_id` with his `profile_id`; the column-missing card disabled, no write).
...
- **Calendar sync** — the `calendar-sync` edge function serves a per-surgeon ICS feed URL (`?surgeon=<CODE>`), matched on `code`, reading `schedule_days`; `verify_jwt` must stay OFF (clients send no auth header) — verify with an unauthenticated GET → 200 + `BEGIN:VCALENDAR`. Subscription instructions in Settings.
  - **All-day runs (v5, 2026-09-25)** — the feed is all-day by default: one all-day event per run of consecutive days (per surgeon: the same role, `Silvis Primary` / `Silvis Backup`; the full-group feed: the same primary AND backup, `P Burchett · B Acton`, OPEN for an empty slot, `<name> (external cover)` for a cover; a day with neither assignment is no event), `DTSTART;VALUE=DATE` = the first day, `DTEND;VALUE=DATE` = the day after the last (exclusive), the times in the description (`07:00 Fri → 07:00 Mon (Central)`; a run of 7 days or more adds the dates, `07:00 Mon 1/4 → 07:00 Mon 1/11 (Central)`, since the weekdays alone would repeat). UIDs `silvis-<start>-<role>@silvis-call` / `silvis-<start>-group@silvis-call` are keyed on the run's start day: a run that grows or shrinks at its end updates in place; a new start day is a new UID (the old event disappears and the new one is added on the next refresh — accepted). The combined feed's Davenport days merge the same way (consecutive days with the same reason; a service week is one bar). Rows are read 14 days before the window, so a run straddling today−60 that started at most 14 days before it keeps its start day (a longer straddling run would still get a start, and a UID, that moves daily - not expected for a Silvis run). `?timed=1` keeps the old format byte-for-byte (one 07:00 → 07:00 event per day and role, `Silvis Primary Call[ - <Name>]`, UIDs `silvis-<day>-<role>@silvis-call`) for anyone who prefers it; the app's own .ics download is all-day only.
...
stated availability`; `not available` (the four East codes `east-busy`, `east-forecast-busy`, `derived-lock`, `derived-lock-held` since Item E3, Faraz 9/25 - the two East categories `East feed busy` / `East-derived week` before it); `caps reached`; `already on call that day`;
...
the Playwright smoke reads the recorded blob write after Accept & Publish and checks the same (since Item E3: no reason
names East). The board's Why column and
unit patterns read this record, and a live preview overlays the slots and weekends it covers (rendered through the same
`openSlotReason`). **Sentences stored before Item E3 (9/25)** may still name `East feed busy` / `East-derived week`: the
client reads every stored sentence through `helpers.openSlotReasonCurrent` (renamed to `not available`, deduplicated, put
back in table order; a current sentence passes byte for byte) - in `boardReasons` (the Why column, in-app Alerts and the
...
- **Calendar day cells:** an East-vacation **marker** beside the vacation dots — a small diamond, distinct from the
  ...
  drawn as is. Nothing is drawn in public mode. Since Item E3 (Faraz 9/25) the markers, their hover bits and the legend
  line are the scheduler's only (`eastDetailsVisible`), and so are the day-editor line and the strip item below.
...
- **Coverage strip:** `unreviewed East vacations: N (Khan)` (`cov-eastvac-unreviewed`, `data-count`) = the person's
  ...
  **scheduler and the person with the East code only** (another surgeon's or the viewer's Time off view has no East
  card, so the item would be a dead end); hidden in public mode and when nobody has an East code. Since Item E3
  (9/25) for the **scheduler only** - Faraz is both, so nothing is lost; the Time off branch of `openEastVacPanel`
  is no longer reached from the strip.
...
`--override` (then a `schedule.override` audit row and the `schedule.day_edit` row's `overrides` carry the reasons - since Item E3, 9/25, the note gets no `[override: …]` tag, and an old one is stripped; a check that throws
